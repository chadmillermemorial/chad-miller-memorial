...
async function loadAppsScriptSecurityPatch() {
  return await readFile(
    new URL("../apps-script/InKindSecurityRouterPatch.gs", import.meta.url),
    "utf8"
  );
}
...
test("Apps Script gates all in-kind actions with the existing internal-key verifier", async () => {
  const source = await loadAppsScriptSecurityPatch();

  assert.match(source, /createInKindDonation/);
  assert.match(source, /uploadInKindDonationFile/);
  assert.match(source, /finalizeInKindDonation/);
  assert.match(source, /requireSponsorCapacityInternalKey_\(data\)/);
  assert.match(source, /SPONSOR_CAPACITY_INTERNAL_KEY/);
});
