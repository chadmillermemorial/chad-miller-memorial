...
async function loadAppsScriptPatch() {
  try {
    return await readFile(
      new URL("../apps-script/AdminRefundPatch.gs", import.meta.url),
      "utf8"
    );
  } catch (error) {
    assert.fail(
      `Expected apps-script/AdminRefundPatch.gs to contain the deployed replacement functions: ${
...
test("public capacity lookup remains lock-free and read-only", async () => {
  const source = await loadAppsScriptPatch();
